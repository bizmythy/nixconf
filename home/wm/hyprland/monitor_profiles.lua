...
local function start_sunshine()
  ...
  local pid = util.capture("sh -c " .. util.shell_quote("sunshine output_name=" .. tostring(monitor_id) .. " >/dev/null 2>&1 & echo $!"))
  if pid ~= nil then
...
function M.choose_profile()
  ...
  local result = util.capture(
    "printf %s " .. util.shell_quote(util.join_lines(labels)) .. " | fuzzel --dmenu --index --prompt " .. util.shell_quote("Monitors> ")
  )
