...
# - Transcription via local whisper.cpp models

# Configuration (see default_xhisperrc or ~/.config/xhisper/xhisperrc):
# - model-path : Optional path to a ggml Whisper model file
# - model-device : Device to use (auto, cpu, gpu)
...
is_silent() {
  ...
  vol_stats=$(ffmpeg -i "$recording" -af "volumedetect" -f null /dev/null 2>&1 | grep -E "mean_volume|max_volume")
  max_vol=$(echo "$vol_stats" | grep "max_volume" | awk '{print $5}')

  # If max volume is below threshold, consider it silent
  # Note: ffmpeg reports in dB, negative values (e.g., -50 dB is quiet)
  if [ -n "$max_vol" ]; then
    is_quiet=$(echo "$max_vol < $silence_threshold" | bc -l)
...
logging_end_and_write_to_logfile() {
  ...
  logging_end=$(date +%s%N)
  time=$(echo "scale=3; ($logging_end - $logging_start) / 1000000000" | bc)
...
post_process() {
  ...
  result=$(echo "$prompt" | timeout "$post_process_timeout" ollama run "$post_process_model" 2>/dev/null | tr -d '\n\r' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
transcribe() {
  ...
  if [ "$LOCAL_MODE" -eq 1 ]; then
    TRANSCRIPT_SCRIPT="$SCRIPT_DIR/xhisper_transcribe"
  else
    TRANSCRIPT_SCRIPT="xhisper_transcribe"
  fi

  # Build command arguments
  local -a cmd_args=(--device "$model_device")
  ...
  # Run transcription
  transcription=$("$TRANSCRIPT_SCRIPT" "$recording" "${cmd_args[@]}" 2>/dev/null)
